...
class SidebarItem(tk.Canvas):
    def __init__(self, parent, text, command, is_active=False):
        super().__init__(parent, height=40, bg=BACKGROUND, highlightthickness=0)
    ...
    def draw(self):
        ...
        if self.is_active:
            ...
            self.create_rounded_rect(5, 2, w-5, h-2, 8, fill=MUTED, outline="")
            fg = "#000000" 
            weight = "bold"
        else:
            fg = MUTED_FOREGROUND
    ...
    # rectangles need more smoothing with a border radius so
    # need to figure out a brute force method for this
...
class Sidebar(ttk.Frame):
    def __init__(self, parent, on_navigate):
        ...
        self.logo_frame = ttk.Frame(self, padding=20)
        ...
        self.menu_frame = ttk.Frame(self, padding=(8, 10))
        ...
        self.user_frame = ttk.Frame(self, padding=8)
        self.user_frame.pack(fill="x", side="bottom")
        
        # placeholder avatar
        self.avatar = tk.Canvas(self.user_frame, width=32, height=32, bg=MUTED, highlightthickness=0)
        self.avatar.create_oval(0, 0, 32, 32, fill=PRIMARY)
        self.avatar.pack(side="left", padx=(0, 10))
        
        self.user_info = ttk.Frame(self.user_frame)
